...
export default function ScheduleCalendarPage() {
  ...
  const fetchScheduleData = async () => {
    try {
      ...
      let allSessions = sessionsResponse.data.data || [];
      
      // For staff members, only show upcoming sessions they're assigned to
      if (user?.role === 'staff') {
        const now = new Date();
        allSessions = allSessions.filter(session => {
          const sessionDate = new Date(session.start_time);
          return sessionDate >= now && session.therapist_id === user.id;
        });
      }
